...
## 5. 性能优化 (Performance Optimization) - V2.1+

为了解决 V2.1 基础版在大数据量下性能下降的问题（主要由于 $N \times C$ 的重复评分计算），引入了 **Lazy Greedy (惰性堆)** 和 **Integer Arithmetic (整数运算)** 优化。

### 5.1 Lazy Greedy (惰性贪心)

**原理**: 利用评分函数的单调性。随着需求被满足，发票的评分只会降低（或不变），不会升高。
**实施**:
*   使用 **Max-Heap (最大堆)** 维护候选发票的评分 `(Score, InvoiceID)`。
*   **Lazy Check**: 每次从堆顶取出发票时，检查其评分是否“过时”（即发票包含的某些 SKU 刚刚被扣减过）。
    *   如果**未过时**，直接选中（无需计算其他发票）。
    *   如果**已过时**，重新计算评分并推回堆中。
*   **收益**: 将每轮迭代的复杂度从 $O(C)$ (扫描所有候选) 降低到接近 $O(\log C)$。

### 5.2 Integer Arithmetic (整数化运算)

**原理**: 评分仅用于排序比较，不需要 BigDecimal 的高精度和对象开销。
**实施**:
*   将所有金额乘以 100 转为 `i64` 或 `u128`。
*   `Score = (EffectiveAmount * 100) + (1000 / Frequency)`
*   **收益**: 避免了数千万次的堆内存分配，极大提升了 CPU 缓存命中率和计算速度。

---
*Updated by Antigravity Agent - 2025-12-20*
